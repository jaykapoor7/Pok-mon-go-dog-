...
async function actor(request: Request) { const token = request.headers.get("authorization")?.replace(/^Bearer\s+/i, ""); const admin = getSupabaseAdmin(); if (!admin || !token) return null; const { data: identity } = await admin.auth.getUser(token); if (!identity.user) return null; const { data: member } = await admin.from("ngo_members").select("ngo_id").eq("user_id", identity.user.id).maybeSingle(); if (!member?.ngo_id) return null; const { data: ngo } = await admin.from("ngos").select("verified").eq("id", member.ngo_id).maybeSingle(); return ngo?.verified ? { admin, ngoId: member.ngo_id as string } : null; }
...
export async function GET(request: Request) {
  const current = await actor(request); if (!current) return NextResponse.json({ error: "Verified partner access required." }, { status: 401 });
