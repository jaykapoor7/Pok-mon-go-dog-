      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-2">
        ...
        <article className="rounded-2xl border border-black/[0.08] p-6 dark:border-white/[0.1] lg:col-span-2">
          ...
          <p className="mt-2 max-w-2xl text-sm text-bark-600 dark:text-bark-300">
            The {reported?.value ?? "—"} figure comes from passive surveillance — cases that reach a hospital and get reported up through the system. The {modelled ? nf(modelled.value) : "—"} figure comes from a peer-reviewed community survey designed to catch the deaths that surveillance misses, mostly in rural areas with poor access to care. The gap between them isn&apos;t a mystery to solve later — it&apos;s the clearest evidence that India&apos;s rabies burden is measured far less than it needs to be.
          </p>
        ...
        <article className="rounded-2xl border border-black/[0.08] p-6 dark:border-white/[0.1]">
          ...
          <h2 className="font-display text-xl font-bold leading-snug tracking-tight">
            {popPoints[0] ? <>{popPoints[0].geo.name} counted the most street dogs of any state with published data — {nf(popPoints[0].value)}.</> : "Population data is limited to a handful of states."}
          </h2>
          <p className="mt-2 text-sm text-bark-600 dark:text-bark-300">From the 20th Livestock Census (2019) — the last official nationwide count. Only {popPoints.length} of 28 states have a citable, compiled figure; the rest would need to come from DAHD&apos;s primary state tables.</p>
          <div className="mt-4"><RankedBars points={popPoints} max={8} /></div>
          <p className="mt-3 flex items-center gap-2 text-[12px] text-bark-400"><SourceBadge type="estimate" /> 20th Livestock Census (2019), compiled by state.</p>
        </article>
