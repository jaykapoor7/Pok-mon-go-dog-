...
function buildStories(cases:PublicCaseStory[],care:Awaited<ReturnType<typeof getPublicCareTimeline>>){
 ...
 return [...byDog.entries()].map(([dogId,rows]):Story=>{const ordered=[...rows].sort((a,b)=>+new Date(b.occurred_at)-+new Date(a.occurred_at)),active=ordered.some(row=>!isClosedStatus(row.status)),outcome=ordered.find(row=>row.outcome)?.outcome??null;return{dogId,latest:ordered[0],cases:ordered,careCount:careByDog.get(dogId)??0,active,outcome}}).sort((a,b)=>Number(b.active)-Number(a.active)||+new Date(b.latest.occurred_at)-+new Date(a.latest.occurred_at));
}
export default async function StoriesPage(){
 const [cases,care]=await Promise.all([getPublicCaseStories(),getPublicCareTimeline()]),stories=buildStories(cases,care),active=stories.filter(s=>s.active),completed=stories.filter(s=>!s.active);
 return <AppShell><main className="min-h-screen bg-[#f4f1e9] text-[#0b1e3d]"><div className="mx-auto max-w-7xl px-4 pb-16 pt-7 sm:px-6 lg:px-8">
  <header className="flex flex-col gap-6 border-b border-[#0b1e3d]/10 pb-8 sm:flex-row sm:items-end sm:justify-between">
   <div><p className="text-[11px] font-bold uppercase tracking-[.16em] text-[#2457ce]">Animal stories</p><h1 className="mt-2 max-w-3xl text-[clamp(2.5rem,6vw,4.8rem)] font-semibold leading-[.91] tracking-[-.065em]">A rescue should not disappear into a spreadsheet.</h1><p className="mt-4 max-w-2xl text-sm leading-6 opacity-55">Each animal keeps one continuous public story: why help was requested, what happened in care, what followed, and where the journey ended.</p></div>
  ...
  <div className="flex flex-wrap gap-x-8 gap-y-2 border-b border-[#0b1e3d]/10 py-5 text-sm"><span><b>{stories.length}</b> <span className="opacity-45">animals with stories</span></span><span><b className="text-[#f05b40]">{active.length}</b> <span className="opacity-45">in progress</span></span><span><b>{completed.length}</b> <span className="opacity-45">completed journeys</span></span></div>

  <StorySection title="In progress" lede="Rescue or care is still open." stories={active} empty="No in-progress stories are currently published."/>
  <StorySection title="Completed journeys" lede="Closed stories with the care and outcome kept attached to the animal." stories={completed} empty="No completed stories are currently published."/>
 </div></main></AppShell>;
}
...
function StoryRow({story,index}:{story:Story;index:number}){
 const row=story.latest,name=row.animal_name||row.animal_code||"Animal record",category=rescueCategory({subtype:row.category,title:row.title,detail:row.outcome}),status=rescueStatusLabel(row.status,row.outcome);
