...
export type KindHourSource = {
  lines: number; profiles: number; atLocality: number; withheld: number;
...
async function read(): Promise<{ source: KindHourSource | null; example: KindHourExample | null; orgSlug: string }> {
  ...
  const m = (src?.metadata ?? {}) as { profiles_at_locality?: number; withheld_lines?: number; register_span?: { from?: string; to?: string } };
  const source: KindHourSource | null = src ? {
    lines: src.record_count ?? 0, profiles: src.published_record_count ?? 0,
    atLocality: m.profiles_at_locality ?? 0, withheld: m.withheld_lines ?? 0,
...
export const getKindHour = unstable_cache(read, ["kind-hour-v1"], { revalidate: 900 });
