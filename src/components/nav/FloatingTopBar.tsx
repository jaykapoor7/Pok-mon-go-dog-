      ...
      <header className="fixed inset-x-0 top-0 z-50 border-b border-black/[0.06] bg-paper/85 backdrop-blur-md dark:border-white/10 dark:bg-ink/85 lg:pl-60">
        <div className="mx-auto flex h-[4.5rem] max-w-3xl items-center gap-2 px-3 lg:max-w-5xl lg:px-6">
          ...
          <button
            ...
            className="flex shrink-0 items-center gap-1.5 rounded-full border border-black/[0.08] bg-white/80 py-1 pl-1 pr-1.5 shadow-card transition-colors hover:bg-white active:scale-[0.98] dark:border-white/10 dark:bg-bark-900/70 sm:pr-2.5"
          >
            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-paw-500 text-sm font-bold text-white">
              {initials ?? <User className="h-5 w-5" />}
            </span>
            <span className="hidden max-w-[6rem] truncate text-sm font-semibold text-bark-800 dark:text-bark-100 sm:block">
              {firstName ?? "Sign in"}
            </span>
