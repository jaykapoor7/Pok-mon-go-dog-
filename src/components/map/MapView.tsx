...
import { rateForPlace } from "@/lib/platform/network";
import {
  markerStateFor,
  fedRecently,
  type MarkerState,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px 14px", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {/* What it costs to sterilise this animal.

            The AWBI figure is a national ceiling, not what any particular
            city pays, and published municipal rates run from Rs 900 to
            Rs 1,650. But nobody looking at one dog should be asked which
            city to price it in: the dog is somewhere, and the record knows
            where. This reads the animal's own location. Choosing a city is
            a question for the costing page, where you are pricing a
            programme rather than looking at an animal. */}
        <CostBlock dog={dog} />
      </div>
...
function CostBlock({ dog }: { dog: Dog }) {
  ...
  return (
    <div style={{ borderTop: `1px solid ${BORDER}`, paddingTop: 12 }}>
      <div style={{ fontSize: 10.5, letterSpacing: "0.11em", color: "rgba(255,255,255,0.66)", marginBottom: 10 }}>
        COST TO STERILISE {rate ? `IN ${rate.city.toUpperCase()}` : ""}
      </div>
      ...
      <p style={{ margin: "0 0 12px", fontSize: 10.5, lineHeight: 1.6, color: "rgba(255,255,255,0.5)", letterSpacing: "0.04em" }}>
        {rate
          ...
          : `AWBI-notified ceiling, ABC (Dogs) Rules ${UNIT_COSTS.sterilisation.year}. No published rate for this area, so this is the national ceiling rather than what is paid here. Not a StrayPaw estimate.`}
      </p>
