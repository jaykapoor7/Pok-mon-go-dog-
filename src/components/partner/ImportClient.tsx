...
export function ImportClient() {
  ...
  async function read(next = file, selectedSheet?: string, keepMapping = true) {
    if (!next) return;
    ...
    try {
      ...
      if (keepMapping && Object.keys(mapping).length) form.append("mapping", JSON.stringify(mapping));
  ...
  return <div className="space-y-5">
    ...
    {preview && !result && <>
      {preview.sheetProfiles?.length>1&&<section className="rounded-xl border border-black/[.08] p-5 dark:border-white/10"><div><p className="text-[11px] font-semibold uppercase tracking-[.13em] text-paw-600">Workbook map</p><h2 className="mt-1 font-semibold text-bark-900 dark:text-bark-50">What StrayPaw thinks each sheet is</h2><p className="mt-1 max-w-2xl text-[13px] text-bark-500">This is a routing suggestion, not a silent import decision. Salary/rent sheets stay separate from animal records; vehicle sheets feed Operations; rescue, care, follow-up, programme and survey sheets can be reviewed in their own context.</p></div><div className="mt-4 divide-y divide-black/[.06] border-y border-black/[.08] dark:divide-white/[.06] dark:border-white/10">{preview.sheetProfiles.map(sheet=><button type="button" key={sheet.name} onClick={()=>void switchSheet(sheet.name)} className={`grid w-full gap-1 px-1 py-3 text-left sm:grid-cols-[minmax(0,1.4fr)_170px_90px_minmax(0,2fr)] sm:items-center ${sheet.name===preview.sheetName?"bg-paw-50/60 dark:bg-paw-900/10":""}`}><b className="truncate text-[13px]">{sheet.name}</b><span className="text-[12px] font-medium">{sheet.label}</span><span className="text-[11px] tabular-nums text-bark-400">{sheet.rows.toLocaleString()} rows</span><span className="text-[11px] leading-4 text-bark-500">{sheet.reason}</span></button>)}</div></section>}

      <section className="rounded-xl border border-black/[.08] p-5 dark:border-white/10"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[11px] font-semibold uppercase tracking-[.13em] text-paw-600">Step 1</p><h2 className="mt-1 font-semibold text-bark-900 dark:text-bark-50">Choose a sheet and map it</h2><p className="mt-1 text-[13px] text-bark-500">{mapped} fields mapped{selectedProfile?` · detected as ${selectedProfile.label.toLowerCase()}`:""}. Empty mappings remain in the raw import and can be mapped later.</p></div><select aria-label="Workbook sheet" value={preview.sheetName} onChange={(e) => void switchSheet(e.target.value)} className="rounded-md border border-black/[.1] bg-transparent px-3 py-2 text-sm dark:border-white/10">{preview.sheetNames.map((name) => <option key={name}>{name}</option>)}</select></div>
        ...
        <button type="button" onClick={() => void read(file, preview.sheetName, true)} disabled={busy} className="mt-4 inline-flex items-center gap-1.5 text-[13px] font-semibold text-paw-600 hover:underline">Refresh preview <ChevronRight className="h-4 w-4" /></button>
      </section>
