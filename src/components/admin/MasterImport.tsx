...
type Preview = { totalRows: number; acceptedRows: number; identityCandidates: number; unidentified: number; counts: Record<string, number>; localityStatus?: LocalityStatus; sheets: Array<{ name: string; headerRow: number; rows: Array<{ sourceRowNumber: number; normalized: { classification: string; classification_reason: string; locality: string | null; event_date: string | null } }> }> };
type Result = { ngo?: string; rowsStaged?: number; alreadyStaged?: boolean; batchIds?: string[]; casesCreated?: number; profilesCreated?: number; medicalEvents?: number; rowsNeedingReview?: number; localityStatus?: LocalityStatus };
...
export function MasterImport({ secret }: { secret: string }) {
  ...
  async function inspectCleanup(execute = false) {
    if (!ngoId) return;
    setBusy(true); setError(null);
    try { const res = await fetch("/api/admin/imports/cleanup", { method: "POST", headers: { Authorization: `Bearer ${secret}`, "Content-Type": "application/json" }, body: JSON.stringify({ ngoId, execute, confirmation: execute ? "REMOVE_BROKEN_V1" : undefined }) }); const data = await res.json(); if (!res.ok) throw new Error(data.error ?? "Could not inspect cleanup."); setCleanup(data.cleaned ?? data.plan); if (!execute) setCleanupArmed(true); else setCleanupArmed(false); } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not inspect cleanup."); } finally { setBusy(false); }
  }
